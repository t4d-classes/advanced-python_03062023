...
from get_rates import (
    get_rates_sync, get_rates_thread, get_rates_threadpool, get_rates_async
)
...
async def main():
    ...
    with rates_api_server(health_check_url):
        ...
        start_time = time.time()
        resp = await get_rates_async(date(2020, 1, 15), date(2020, 2, 10))
        #print(f"get_rates_async responses length: {resp_len}")
        end_time = time.time()
        print(f"get_rates_async: {end_time-start_time}")        
        # for r in resp:
        #     print(r)            
